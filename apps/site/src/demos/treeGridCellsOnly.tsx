...
import { treegridAxis, useTreegridPattern, useTreegridReducer } from '@interactive-os/aria-kernel/patterns'
      ...
      {items.map((item) => {
        ...
        return (
          <div
            key={item.id}
            {...rowProps(item.id)}
            className="grid grid-cols-[minmax(0,1.5fr)_88px_132px] items-center border-b border-stone-100 last:border-b-0"
          >
            <span
