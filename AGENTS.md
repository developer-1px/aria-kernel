...
- This repo should stay specialized to reusable ARIA behavior.
- Consumer harness apps and app-only helper packages live in the sibling workspace `../aria-kernel-apps`.
- Do not add apps, routes, Tailwind presentation, resource adapters, site source viewers, or devtools packages here.
- Reusable ARIA behavior belongs in `packages/aria-kernel`.
...
- App presentation belongs in `../aria-kernel-apps`, not this repo.
...
- Backend-specific details stay out of `@interactive-os/aria-kernel` imports.
...
- New routes, demos, and harness apps belong in `../aria-kernel-apps`.
- Do not hide reusable contracts in app widgets.
...
```bash
rg "removed-package-or-route-name" AGENTS.md README.md CANONICAL.md CONTEXT-MAP.md packages docs
```
